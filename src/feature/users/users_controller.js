...
export async function getAll(req, res) {
    try {
        let item = await service.getAllUsers();
        console.log(item);
        return res.status(item.errCode).json(item);
    } catch (error) {
...
export async function getById(req, res) {
    ...
    try {
        ...
        if (item) {
            ...
            return res.status(HttpStatusCode.NOT_FOUND).json({message:'users not found with this ID'});
        }
    } catch (error) {
        return res.status(HttpStatusCode.INTERNAL_SERVER).json(error);
    }
